...
<head>
    ...
    <meta charset="UTF-8">
    <base href="<?php echo rtrim(APP_URL, '/') . '/' ?>">
    ...
    <script>
        // Initialize cart and favorites when the page loads
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Initialize favorites count if user is logged in
            <?php if (isLoggedIn()): ?>
            ...
            .then(data => {
                if (data && data.success) {
                    ...
                    if (favoritesCountElement) {
                        ...
                        favoritesCountElement.style.display = favoritesCount > 0 ? 'inline-block' : 'none';
                    }
                }
            })
            .catch(error => {
                console.error('Error initializing favorites count:', error);
            });
            <?php endif; ?>
        });
    </script>
...
<body>
    ...
    <div class="container my-4">
        <!-- Categorías en un panel desplegable -->
        <div class="mb-4">
            <div class="collapse mt-2" id="categoriasCollapse">
                <div class="card card-body p-0">
                    <div class="list-group list-group-flush">
                        <?php foreach ($categories as $category): ?>
                            <a href="<?= APP_URL ?>/categoria.php?id=<?= $category['id'] ?>" 
                               class="list-group-item list-group-item-action <?= (isset($categoria_id) && $categoria_id == $category['id']) ? 'active' : '' ?>">
                                <?= htmlspecialchars($category['nombre']) ?>
                            </a>
                        <?php endforeach; ?>
                    </div>
